...
The 1,036 female cases with ductal and lobular neoplasms breast cancer contributed with **1,164** RNA-seq, HTSeq Counts (raw counts) samples/files. 
...
3. Standard `DESeq2 (1.28.1)` analysis was run as illustrated in the common package vignettes. 
   ...
   4. A DESeqResults object was retried using `results()` function. Only 285 out of the 310 repair genes were found in the results. The p-value and LFC of the study were specified at this step (lfcThreshold=1; p-value=0.05).
4. Significant DEGs were collected with LFC > 1 or less than < -1 and with adjusted p-value < 0.05. Only 35 genes out of the 285 repair genes were significant on these conditions; all are up regulated.
