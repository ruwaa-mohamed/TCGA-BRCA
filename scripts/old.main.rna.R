...
# BiocManager::install("genefilter")# packageVersion: 1.66.0    1.70.0    # already instaled with DESeq2
...
################################################################################
## working with the new sample sheet and dropping metastatic
females.ductalandlobular.sample.sheet <- read.csv("./raw_data/females_only.ductal-and-lobular-only.gdc_sample_sheet.2020-06-17.tsv", sep="\t", header=TRUE)
table(females.ductalandlobular.sample.sheet$Sample.Type)
females.ductalandlobular.sample.sheet.tumandnormal <- females.ductalandlobular.sample.sheet[! females.ductalandlobular.sample.sheet$Sample.Type == "Metastatic",]
rna.exp.df.females.ductalandlobular.tumandnormal <- rna.exp.df[, colnames(rna.exp.df) %in% females.ductalandlobular.sample.sheet.tumandnormal$File.ID]
saveRDS(rna.exp.df.females.ductalandlobular.tumandnormal, file = "./saved_objects/exp.rna.subsetted.rds")
################################################################################
## Changing ENSG to SYMBOL
rna.exp.df.sym <- ENSG_to_symbol(rna.exp.df.females.ductalandlobular.tumandnormal) ## changed from rna.exp.df
saveRDS(rna.exp.df.sym, file = "./saved_objects/rna.exp.df.sym.rds")
rna.exp.df.sym <- readRDS(file = "./saved_objects/rna.exp.df.sym.rds")
## 60,483 ENSG where reduced to 25,393 gene symbol (org.Hs.eg.db version 3.8.2)
## 60,483 ENS to 25,596 gene symbol (org.Hs.eg.db version 3.11.4)
...
## After second aggregation, 25,531
################################################################################
...
col.data <- females.ductalandlobular.sample.sheet.tumandnormal[, colnames(females.ductalandlobular.sample.sheet.tumandnormal) %in% c("File.ID", "Sample.Type", "Sample.ID")]
...
rownames(count.data.sorted) <- rownames(rna.exp.df.sym.agg)
